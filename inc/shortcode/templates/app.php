...
<div id="kk-session-banner" class="kk-session-banner" style="display:none" role="alert">                                                                    <span>Din session har gått ut.</span>                                            <button type="button" onclick="window.location.reload()">Logga in igen</button>  </div> 
...
const camFlip   = document.getElementById('kk-camFlip');
  ...
  async function maybeRedirectToLogin(response){
    ...
    const banner = document.getElementById('kk-session-banner');
  ...
  async function fetchJSON(url){
    ...
    if(!r.ok){
      ...
      if (js.err === 'ip_banned') {
        ...
        alert('Regelbrott - Bannad' + suffix);
        location.reload();
        return [];
      }
...
function renderList(el, items, options = {}){
  ...
  items.forEach(m=>{
    ...
    if (allowOlder) {
      ...
      // Skip messages already confirmed in the DOM (e.g. optimistic sends that
      // were finalized before the poll returned them). Without this check the
      // cursor in finalizePendingMessage would have to jump forward to avoid
      // duplicates, which inadvertently skips messages from other users that
      // arrived between the last poll and the send.
      const existing = el.querySelector(`li.item[data-id="${mid}"]`);
      if (existing && !existing.dataset.temp) return;
    }
...
function normalizeUnreadMap(map) {
  ...
  function sortUsersForList(){
    ...
    return [...USERS].sort((a,b)=>{
      ...
      const guestRe = /-guest$/i;
      const ga = guestRe.test(String(a.name || '').trim());
      const gb = guestRe.test(String(b.name || '').trim());
...
function finalizePendingMessage(pending, payload){
  ...
  if (Number.isFinite(mid) && mid > 0) {
    ...
    // Do NOT advance pubList.dataset.last here. The poll cursor must only move
    // forward based on messages actually received from the server. Jumping the
    // cursor to the sent message's ID would skip any messages from other users
    // that arrived between the last poll and this send (their IDs would fall
    // below mid and would never be fetched again). Deduplication of the sent
    // message on the next poll is handled inside renderList instead.
