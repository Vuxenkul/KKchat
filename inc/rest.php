...
add_action('rest_api_init', function () {
  ...
  register_rest_route($ns, '/video/upload-url', [
    ...
    'callback' => function (WP_REST_Request $req) {
      ...
      $durationRaw = $req->get_param('duration');
      $widthRaw    = $req->get_param('width');
      $heightRaw   = $req->get_param('height');
      $posterMime  = strtolower(trim((string) $req->get_param('poster_mime')));
      $posterBytes = (int) $req->get_param('poster_bytes');
      ...
      $durationVal = $durationRaw !== null ? (float) $durationRaw : 0.0;
      if ($durationVal <= 0) {
        kkchat_json(['ok' => false, 'err' => 'bad_duration'], 400);
      }

      $widthVal  = (int) $widthRaw;
      $heightVal = (int) $heightRaw;
      if ($widthVal <= 0 || $heightVal <= 0) {
        kkchat_json(['ok' => false, 'err' => 'bad_dimensions'], 400);
      }

      $thumbAllowed = array_map('strtolower', (array) apply_filters('kkchat_video_thumbnail_mimes', ['image/jpeg', 'image/png']));
      $thumbLimit   = (int) apply_filters('kkchat_video_thumbnail_max_bytes', 512 * 1024);
      if ($posterMime === '' || !in_array($posterMime, $thumbAllowed, true)) {
        kkchat_json(['ok' => false, 'err' => 'bad_thumbnail_type'], 400);
      }
      if ($posterBytes <= 0 || ($thumbLimit > 0 && $posterBytes > $thumbLimit)) {
        kkchat_json(['ok' => false, 'err' => 'bad_thumbnail_size', 'max' => $thumbLimit], 400);
      }
      ...
      $thumbSigned = kkchat_video_presign($cfg, 'PUT', $thumb_key, [
        'headers' => ['Content-Type' => $posterMime ?: 'image/jpeg'],
        'expires' => max(60, (int) ($cfg['presign_ttl'] ?? 900)),
      ]);
      if (is_wp_error($thumbSigned)) {
        kkchat_json(['ok' => false, 'err' => $thumbSigned->get_error_code()], 500);
      }
      ...
      $wpdb->insert($t['videos'], [
        ...
        'duration_seconds' => round($durationVal, 2),
        'width_px'         => $widthVal,
        'height_px'        => $heightVal,
        'thumbnail_key'    => $thumb_key,
        'thumbnail_url'    => $thumb_url,
        'thumbnail_mime'   => $posterMime,
        'thumbnail_bytes'  => $posterBytes,
        'upload_token_hash'=> $token_hash,
        'upload_expires_at'=> $expires_at,
        'public_url'       => $public_url,
      ], ['%d','%d','%d','%s','%d','%s','%s','%s','%d','%d','%s','%s','%s','%d','%s','%d','%s']);
      ...
      $assetPayload = kkchat_video_asset_payload([
        'id'              => $asset_id,
        'status'          => 'pending_upload',
        'object_key'      => $key,
        'public_url'      => $public_url,
      ...
      kkchat_json([
        ...
        'thumbnail_upload' => [
          'url'     => $thumbSigned['url'],
          'method'  => 'PUT',
          'headers' => $thumbSigned['headers'] + ['Content-Type' => $posterMime ?: 'image/jpeg'],
          'expires' => $expires_at,
        ],
        'asset'  => [
          'id'        => $assetPayload['id'],
          ...
          'duration'  => $assetPayload['duration'],
