...
function kkchat_maybe_schedule_schema_upgrade(): void {
  $default_interval = HOUR_IN_SECONDS;
  $interval         = (int) apply_filters('kkchat_schema_check_interval', $default_interval);
  ...
  $now  = time();
  $last = (int) get_option('kkchat_schema_last_check', 0);

  if ($last && ($now - $last) < $interval) {
    return;
  }

  // Persist immediately to avoid a thundering herd of concurrent checks.
  update_option('kkchat_schema_last_check', $now);

  kkchat_maybe_upgrade_schema();
}
add_action('plugins_loaded', 'kkchat_maybe_schedule_schema_upgrade', 20);
...
function kkchat_maybe_migrate(){
  ...
  update_option('kkchat_db_version', KKCHAT_DB_VERSION);
    // Force an immediate schema re-check after upgrades so new columns/indexes are applied promptly.
  delete_option('kkchat_schema_last_check');
}
