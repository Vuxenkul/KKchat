...
function kkchat_admin_settings_page() {
  ...
  $settings_export = [
    'kkchat_sync_enabled',
    'kkchat_sync_concurrency',
    'kkchat_sync_breaker_threshold',
    'kkchat_sync_breaker_window',
    'kkchat_sync_breaker_cooldown',
    'kkchat_dupe_window_seconds',
    'kkchat_dupe_fast_seconds',
    'kkchat_dupe_max_repeats',
    'kkchat_min_interval_seconds',
    'kkchat_dedupe_window',
    'kkchat_report_autoban_threshold',
    'kkchat_report_autoban_window_days',
    'kkchat_report_reason_rules',
    'kkchat_poll_hidden_threshold',
    'kkchat_poll_hidden_delay',
    'kkchat_poll_hot_interval',
    'kkchat_poll_medium_interval',
    'kkchat_poll_slow_interval',
    'kkchat_poll_medium_after',
    'kkchat_poll_slow_after',
    'kkchat_poll_extra_2g',
    'kkchat_poll_extra_3g',
    'kkchat_poll_fallback_interval',
    'kkchat_public_presence_cache_ttl',
    'kkchat_admin_presence_cache_ttl',
    'kkchat_admin_auto_incognito',
    'kkchat_first_load_limit',
    'kkchat_first_load_exclude_banners',
    'kkchat_sync_metrics',
  ];

  // Exportera nuvarande inställningar som JSON
  if (isset($_POST['kk_download_settings'])) {
    ...
    foreach ($settings_export as $option_name) {
      $export_data['settings'][$option_name] = get_option($option_name);
    }

    $filename = 'kkchat-settings-' . gmdate('Y-m-d-His') . '.json';
  ...
  if (!is_array($sync_metrics)) {
    $sync_metrics = [];
  }
  $sync_metrics = array_merge($sync_metrics_defaults, $sync_metrics);

  $sync_last_request_at = (int) $sync_metrics['last_request_at'];
  $sync_last_request_at_display = $sync_last_request_at > 0
    ? date_i18n('Y-m-d H:i:s', $sync_last_request_at)
    : '–';
  ...
  <div class="wrap">
    ...
    <form method="post">
      ...
      <p>
        <button class="button button-primary" name="kk_save_settings" value="1">Spara</button>
        <button class="button" name="kk_download_settings" value="1">Exportera inställningar (.json)</button>
      </p>
    </form>
  </div>
  <?php
