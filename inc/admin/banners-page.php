...
/**
 * Option 1 helpers: render row + table (so you don't duplicate markup)
 */
if (!function_exists('kkchat_admin_render_banner_row')) {
  function kkchat_admin_render_banner_row($b, string $banners_base, string $nonce_key): void {
    $run_url  = wp_nonce_url(add_query_arg('kk_run_now', (int)$b->id, $banners_base), $nonce_key);
    $tog_url  = wp_nonce_url(add_query_arg('kk_toggle',  (int)$b->id, $banners_base), $nonce_key);
    $del_url  = wp_nonce_url(add_query_arg('kk_delete',  (int)$b->id, $banners_base), $nonce_key);
    $edit_url = add_query_arg('kk_edit', (int)$b->id, $banners_base);
...
if (!function_exists('kkchat_admin_render_banners_table')) {
  function kkchat_admin_render_banners_table(array $list, string $banners_base, string $nonce_key, string $emptyText): void {
    ?>
    <table class="widefat striped">
      ...
      <tbody>
        <?php if ($list): foreach ($list as $b): ?>
          <?php kkchat_admin_render_banner_row($b, $banners_base, $nonce_key); ?>
        <?php endforeach; else: ?>
          <tr><td colspan="7"><?php echo esc_html($emptyText); ?></td></tr>
        <?php endif; ?>
      </tbody>
    </table>
    <?php
...
function kkchat_admin_banners_page(){
  ...
  // Option 1: split into active/inactive lists
  $active_list   = $wpdb->get_results("SELECT * FROM {$t['banners']} WHERE active = 1 ORDER BY id DESC");
  $inactive_list = $wpdb->get_results("SELECT * FROM {$t['banners']} WHERE active = 0 ORDER BY id DESC");
  ...
  <div class="wrap">
    ...
    <h2>Aktiva banderoller</h2>
    <?php kkchat_admin_render_banners_table((array)$active_list, $banners_base, $nonce_key, 'Inga aktiva banderollscheman ännu.'); ?>

    <h2 style="margin-top:22px;">Inaktiva banderoller</h2>
    <?php kkchat_admin_render_banners_table((array)$inactive_list, $banners_base, $nonce_key, 'Inga inaktiva banderollscheman ännu.'); ?>
  </div>
  <script>
