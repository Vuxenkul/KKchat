        ...
        <section id="vPublic" class="view" active>
          <div class="msgwrap">
            ...
            <form id="kk-pubForm" class="inputbar">
              ...
              <!-- Hidden inputs (upload picker and camera capture) -->
...
async function doLogout(){
  ...
  if ((m.kind||'chat') === 'video'){
    return `<li class="item ${sid===ME_ID?'me':'them'}${roleClass}" data-id="${mid}" data-sid="${sid}" data-sname="${escAttr(who)}">
      <div class="bubble">[video-meddelanden stöds inte längre]</div>
      ${metaHTML}
    </li>`;
  }
  ...
  items.forEach(m=>{
    ...
    if ((m.kind||'chat') === 'banner'){
      ...
      if ((m.kind||'chat') === 'image') {
        ...
        bubbleHTML = `<div class="bubble">[video-meddelanden stöds inte längre]</div>`;
      } else {
        const txt = String(m.content||'');
...
function coerceLastMessage(raw){
  ...
  if (!meaningfulText && !normalized.room && !normalized.to && kindLower !== 'image') {
    return null;
  }
 ...
 function renderLogRows(rows){
  ...
  rows.forEach(m => {
    ...
    if ((m.kind||'chat') === 'image') {
      ...
      contentHTML = `<div>[video-meddelanden stöds inte längre]</div>`;
    } else {
