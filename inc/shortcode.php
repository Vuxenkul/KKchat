...
const VIDEO_THUMB_MAX_BYTES  = 320 * 1024;
...
function videoMetaText(data){
  ...
  const resolution = formatVideoResolution(data.width, data.height);
...
async function seekVideoFrame(video, time){
  ...
  const maxTime = Number.isFinite(total) && total > 0 ? Math.max(0, total - 0.05) : 0;
  ...
  return new Promise((resolve, reject) => {
    ...
    const onLoadedData = () => { if (target === 0) finish(); };
    const onError = () => {
      cleanup();
      reject(new Error('Kunde inte läsa videoramen'));
    };
    ...
    try {
      ...
      cleanup();
      reject(new Error('Kunde inte förhandsgranska videon'));
    }
  });
}

async function prepareVideoUpload(file){
  if (!file) throw new Error('Ingen video vald');
  if (!(file instanceof File)) throw new Error('Ogiltig video');
  ...
  if (type && typeof video.canPlayType === 'function') {
    const support = video.canPlayType(type);
    if (!support) {
      throw new Error('Formatet verkar inte stödjas av din webbläsare.');
    }
  }
  ...
  try {
    await new Promise((resolve, reject) => {
      ...
      const onError = () => { cleanup(); reject(new Error('Kunde inte läsa videon')); };
    ...
    if (!Number.isFinite(duration) || duration <= 0) {
      throw new Error('Kunde inte fastställa videons längd.');
    }
    if (width <= 0 || height <= 0) {
      throw new Error('Kunde inte fastställa videons upplösning.');
    }

    const maxSide = Math.max(width, height);
    const preferred = duration > 1 ? duration / 2 : duration;
    try {
      await seekVideoFrame(video, preferred);
    } catch (_) {
      await seekVideoFrame(video, 0);
    }

    const scale = maxSide > 640 ? 640 / maxSide : 1;
    const canvas = document.createElement('canvas');
    ...
    const blob = await canvasToSizedBlob(canvas, 'image/jpeg', VIDEO_THUMB_MAX_BYTES, 0.82, 0.6);
    if (!blob) {
      throw new Error('Kunde inte skapa miniatyr.');
    }
    if (blob.size > VIDEO_THUMB_MAX_BYTES) {
      throw new Error('Miniatyren kunde inte komprimeras tillräckligt.');
    }

    const thumbFile = new File([blob], (file.name || 'video') + '-thumb.jpg', { type: 'image/jpeg' });
    ...
    return {
      ...
      thumbnailMime: thumbFile.type || 'image/jpeg',
      thumbnailBytes: thumbFile.size,
    };
  } finally {
...
async function createVideoUploadSession(file, meta = {}){
  const payload = {
    csrf_token: CSRF,
  ...
  if (Number.isFinite(duration) && duration > 0) {
    payload.duration = Number(duration.toFixed(2));
  }
  ...
  const thumbBytes = Number(meta?.thumbnailBytes);
  if (Number.isFinite(thumbBytes) && thumbBytes > 0) {
    payload.poster_bytes = Math.round(thumbBytes);
  }
  const thumbMime = meta?.thumbnailMime ? String(meta.thumbnailMime) : '';
  if (thumbMime) {
    payload.poster_mime = thumbMime;
  }
...
async function uploadVideoThumbnail(upload, file){
  if (!upload || !upload.url) throw new Error('Saknar miniatyr-URL');
  const headers = Object.assign({}, upload.headers || {});
  if (!headers['Content-Type']) headers['Content-Type'] = file?.type || 'image/jpeg';
  const method = (upload.method || 'PUT').toUpperCase();
  const resp = await fetch(upload.url, { method, headers, body: file });
  if (!resp.ok){
    throw new Error('Kunde inte ladda upp miniatyr');
  }
  return true;
}
...
pubVideoInp?.addEventListener('change', async () => {
  ...
  let prep;
  try {
    showToast('Förhandsgranskar video…');
    prep = await prepareVideoUpload(file);
  } catch (e) {
    showToast(e?.message || 'Kunde inte läsa videon');
    return;
  }

  try {
    showToast('Förbereder video…');
    const session = await createVideoUploadSession(file, prep);
    const assetId = Number(session?.asset?.id || 0);
    const token   = session?.token || '';
    if (!assetId || !token) throw new Error('Saknar videoinformation');
    if (!prep?.thumbnailFile || !session?.thumbnail_upload) {
      throw new Error('Saknar miniatyrinformation');
    }
    showToast('Laddar upp miniatyr…');
    await uploadVideoThumbnail(session.thumbnail_upload, prep.thumbnailFile);
